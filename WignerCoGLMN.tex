...
\begin{center}
{\large\bf Fundamental Wigner coefficients for unitary representations
...
$$
that satisify the polynomial identities
\begin{align}
...
Setting $i=j=m+n$ gives
\begin{align}
\left\langle\left. \left(\begin{array}{c} \Lambda\\ {[\Lambda'_0]} \end{array}\right) \right|\right.
\bar{c}_r \left.\left| \left(\begin{array}{c} \Lambda\\
{[\Lambda_0]} \end{array}\right)
\right. \right\rangle = \delta_{[\Lambda'_0][\Lambda]}
\left| \left\langle\left. 
\begin{array}{c} \Lambda+\varepsilon_r\\ {[\Lambda_0]} \end{array}
\right|\right.
\left.
e_{m+n}\otimes \begin{array}{c} \Lambda \\
{[\Lambda_0]} \end{array}
\right\rangle \nn \right|^2,
\end{align}
while from the analogeous argument for the projections $P[r]$ we have
\begin{align}
\left\langle\left. \left(\begin{array}{c} \Lambda\\ {[\Lambda'_0]} \end{array}\right) \right|\right.
c_r \left.\left| \left(\begin{array}{c} \Lambda\\
{[\Lambda_0]} \end{array}\right)
\right. \right\rangle = \delta_{[\Lambda'_0][\Lambda]}
\left| \left\langle\left. 
\begin{array}{c} \Lambda-\varepsilon_r\\ {[\Lambda_0]} \end{array}
\right|\right.
\left.
\bar{e}_{m+n}\otimes \begin{array}{c} \Lambda \\
{[\Lambda_0]} \end{array}
\right\rangle \nn \right|^2,
\end{align}
...
$$ 
\bar{C}_r = \bar{Q}[r]_{m+n+1}^{\ m+n+1}, ~~C_r = Q[r]_{\ m+n+1}^{m+n+1}
$$
are the $gl(m|n+1)$ analogues of the operators $\bar{c}_r$ and $c_r$ which correspond to the squares of Wigner coefficients.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


\section{Wigner coefficients} 
\label{Wigner}
...
\begin{align}
\bar{c}_s = \prod_{k\in \tilde{I}',k\neq s} \left(\bar{\beta}_s - \bar{\beta}_k\right)^{-1}\prod_{r\in
I'} \left(\bar{\beta}_s - \bar{\alpha}_r - (-1)^{(r)}\right),\ \ s\in \tilde{I}'
\end{align}
and
\begin{align}
c_s = \prod_{k\in\tilde{I},k\neq s} (\beta_s - \beta_k)^{-1}\prod_{r\in I}(\beta_s -
\alpha_r-(-1)^{(r)}),\ \ s\in \tilde{I}.
\end{align}
...
The definitions of the projections $\bar{Q}[r]$ and $\bar{P}[r]$ allow the calculation of the invariant $\bar{\rho}_{ru}$ in the following expression \cite{GIW2}
\begin{align}
(\bar{P}[u]\bar{Q}[r]\bar{P}[u])_p^{\ q} &= \bar{\rho}_{ru}\bar{P}[u]_p^{\ q}
\label{equ6.3}
\end{align}
where the $\bar{\rho}_{ru}$ is a $gl(m|n)$ invariant operator given by
\begin{align}
...
\delta_r &= (-1)^{|I|} \prod_{s\in I,s\neq r} \left(\alpha_r - \alpha_s -
(-1)^{(s)}\right)^{-1}\prod_{q\in\tilde{I}} \left(\beta_q - \alpha_r \right),\ \ r\in I',
\end{align}
