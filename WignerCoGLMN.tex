...
The tensor operator $\bar{\mathcal{A}}$ constructed in \cite{GIW1} plays an important role in what follows and is defined as the $m+n$ square matrix with entries
$$
...
- \bar{\alpha}_k} \right)\nn.
\end{align*}
We will now show that the eigenvalues of the invariant
\begin{equation*}
\bar{c}_r = \bar{Q}[r]_{m+n}^{\ m+n}
\end{equation*}
are essentially squares of reduced Wigner coefficients.

Letting $e_i$ denote the Gelfand-Tsetlin (GT) basis states for the vector module $V(\varepsilon_1)$ and ${e^\Lambda_\beta}$ denote the (GT) basis states for the irreducible module $V(\Lambda)$ of highest weight $\Lambda$. Then the matrix elements of $\bar{P}[r]$ can be given in the form
\begin{align*}
